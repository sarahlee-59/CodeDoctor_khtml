...
try:
    ...
    # 인코딩을 순차적으로 시도
    try:
        sales_df = pd.read_csv(os.path.join(project_root, "data/추정매출-상권_2024년.csv"), encoding="utf-8-sig")
    except UnicodeDecodeError:
        sales_df = pd.read_csv(os.path.join(project_root, "data/추정매출-상권_2024년.csv"), encoding="cp949")

    try:
        info_df = pd.read_csv(os.path.join(project_root, "data/상권정보_20221212.csv"), encoding="utf-8-sig")
    except UnicodeDecodeError:
        info_df = pd.read_csv(os.path.join(project_root, "data/상권정보_20221212.csv"), encoding="cp949")

    try:
        change_df = pd.read_csv(os.path.join(project_root, "data/상권변화지표-상권.csv"), encoding="utf-8-sig")
    except UnicodeDecodeError:
        change_df = pd.read_csv(os.path.join(project_root, "data/상권변화지표-상권.csv"), encoding="cp949")

    print("📋 원본 info_df 컬럼:", info_df.columns.tolist())
    info_df.columns = info_df.columns.str.strip()
    print("📋 정리된 info_df 컬럼:", info_df.columns.tolist())

    # 필요한 컬럼만 선택
    info_df = info_df[["상권명", "위도", "경도"]]
